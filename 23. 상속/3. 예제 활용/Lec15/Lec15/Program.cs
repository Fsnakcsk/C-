...
namespace Lec15
{
    class Matrix
    {
        ...
        virtual public void readDataFromConsole()
        {
            int row = m_row;
            int col = m_col;
            double[][] p = m_ptr;
    ...
    class ProductMatrix : Matrix
    {
        ...
        public ProductMatrix(int row, int col)
            : base(row, col)
        {
        }
        public override void readDataFromConsole()
        {
            ...
            for (i = 0; i < row; i++)
            {
                ...
                Console.Write("   냉장고는 몇대 팔았는고? ");
                p[i][0] = Double.Parse(Console.ReadLine());
                Console.Write("   에어콘은 몇대 팔았는고? ");
                p[i][1] = Double.Parse(Console.ReadLine());
                Console.Write("   선풍기는 몇대 팔았는고? ");
    ...
    class TestProductMatrix
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("메트릭스 한번 사용해 봅세!!!");
            Console.Write("사원 수는 몇명인고? ");
            m = int.Parse(Console.ReadLine());
            Console.WriteLine("각 사원들의 매출 수를 입력해주소.");
            ...
            for (i = 0; i < C.row(); i++)
            {
                ...
                Console.WriteLine((int)C.get(i, 0) + "만원 어치 파셨구만.");
            }
        }
    }
}
